...
class ThemeViewModel with ChangeNotifier {
  ...
  Future<ThemePreference> get _currentThemePreference async {
    ...
    final String preferredBrightness = await (preference
        .getString(KEY_PREFERRED_BRIGHTNESS, defaultValue: ''));
    if (preferredBrightness.isEmpty) {
      brightness = MediaQueryData.fromWindow(WidgetsBinding.instance.window)
          .platformBrightness;
    } else {
      brightness = _brightness.entries
          .firstWhere((element) => element.value == preferredBrightness)
          .key;
    }
  ...
  toggleBrightness() {
    final String companyName = _companies.entries
        .firstWhere((e) => e.key == _currentCompanyName)
        .value;
  ...
  updateCompany(CompanyName name) {
    ...
    final themePreference =
        ThemePreference(companyName, _currentTheme.brightness);
    _buildTheme(themePreference);
    notifyListeners();
  }
